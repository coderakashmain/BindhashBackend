...
router.get("/fetch", async (req, res) => {
    ...
    try {
        // Ensure you await and extract only rows
        const [stories] = await db.execute(
            `SELECT * FROM stories WHERE (view_privacy = 'public' OR JSON_CONTAINS(allowed_users, ?, '$'))
...
router.post("/vote", async (req, res) => {
    ...
    try {
        ...
        if (existingVote.length > 0) {
            return res.status(400).json({ error: "You have already voted on this story" });
        }
